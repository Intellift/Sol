 ...
 */
public abstract class SimpleCrudApiClient<E extends Identifiable<ID>, D extends Identifiable<ID>, ID extends Serializable> extends CrudApiClient<E, D, ID> {

    public SimpleCrudApiClient(AsyncRestOperations asyncRestOperations) {
        super(asyncRestOperations);
    }

    @SafeVarargs
    public final Future<ResponseEntity<List<D>>> getAll(final Tuple2<String, List<String>>... parameters) {
        ...
        return convert(asyncRestOperations.exchange(
                ...
                HttpMethod.GET,
                httpEntity,
                new CustomParameterizedTypeReference<List<D>>(getDtoClass()) {
                }
        ));
    }
}
